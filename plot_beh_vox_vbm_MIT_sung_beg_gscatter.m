%% Grey Matter Volume in Baseline Chronic Stroke with Aphasia: data visualization for MIT of phrases in sung format 
% *BACKGROUND*
% 
...
% was set with* words per minute of MIT phrases in sung format from the onset of the patient?s production until the end *
% 
%% User defined inputs
...
peak_path='G:\Aphasia_project\VBM_v4\analysis\L2\MIT\GMV_WPM_MIT_Sung_Total_Dur_Beg_TIV_Age_TP1\';
...
% Save peak Voxel in mm and loop for the clusters from different L2 analysis
        cd(peak_path)
        peaks_xyz=xlsread('WPM_MIT_Sung_Total_Dur_Beg_TIV_Age_p001unc_k50', 2, 'I2:K6');
        % Loop for patients
...
%%
WPM_MIT_Sung_Total_Dur_Beg_TP1_N45= load(fullfile(covbeh_path,'Beh_MIT_WPM_Sung_Total_Dur_Beg_TP1_N45.mat'));
Group=xlsread(fullfile(group_path,'LASA_aphasia_group.xlsx'));
% Remove patients with NaN values in behaviour
if sum(isnan(WPM_MIT_Sung_Total_Dur_Beg_TP1_N45.beh_cov_MIT(:,2)))~=0
    NAN_Index=find(isnan(WPM_MIT_Sung_Total_Dur_Beg_TP1_N45.beh_cov_MIT(:,2)));
    WPM_MIT_Sung_Total_Dur_Beg_TP1_N45.beh_cov_MIT(NAN_Index,:)=[];
    Group(NAN_Index,:)=[];
...
for p=1:size(peaks_xyz,1)
    if exist('pa_int_fixed')
        x=WPM_MIT_Sung_Total_Dur_Beg_TP1_N45.beh_cov_MIT(:,2);
        y=cell2mat(pa_int_fixed(:,p+1));
        g=Group(:,5);
        labels=pa_int_fixed(:,1);
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs MIT SUNG'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM MIT SUNG')
        cd(output_path)
        saveas(gcf,['gscatter_VBM_WPM_MIT_Sung_Total_Dur_Beg_Cluster_' num2str(p) '.jpg'])
        % label data points
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs MIT SUNG WITH PATIENTS ID'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM MIT SUNG')
        for k=1:length(labels)
            text (x(k,1), y(k,1), labels {k,1}(5:6),'fontSize',8,'VerticalAlignment','top', 'HorizontalAlignment','left');
        end
        saveas(gcf,['gscatter_VBM_WPM_MIT_Sung_Total_Dur_Beg_Cluster_' num2str(p) '_labels.jpg'])
    else
%% Plot Intensity against behaviour for the peak coordinate
%%
        x=WPM_MIT_Sung_Total_Dur_Beg_TP1_N45.beh_cov_MIT(:,2);
