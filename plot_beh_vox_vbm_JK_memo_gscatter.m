%% Grey Matter Volume in Baseline Chronic Stroke with Aphasia: data visualization for singing Jaako Kulta from memory
% *BACKGROUND*
% 
...
% was set with* words per minute of spontaneous singing from memory Jaakko Kulta average of 1 & 2 production. *
% 
%% User defined inputs
...
peak_path='G:\Aphasia_project\VBM_v4\analysis\L2\singing\GMV_WPM_Singing_Memo_TIV_Age_TP1\';
...
% Save peak Voxel in mm and loop for the clusters from different L2 analysis
        cd(peak_path)
        peaks_xyz=xlsread('WPM_Singing_Memo_TIV_Age_p001unc_k50', 2, 'I2:K3');
        % Loop for patients
...
%%
WPM_JK12_Memo_TP1_N45= load(fullfile(covbeh_path,'Beh_singing_WPM_JK12_Memo_TP1_N45.mat'));
Group=xlsread(fullfile(group_path,'LASA_aphasia_group.xlsx'));
% Remove patients with NaN values in behaviour
if sum(isnan(WPM_JK12_Memo_TP1_N45.beh_cov_singing(:,2)))~=0
    NAN_Index=find(isnan(WPM_JK12_Memo_TP1_N45.beh_cov_singing(:,2)));
    WPM_JK12_Memo_TP1_N45.beh_cov_singing(NAN_Index,:)=[];
    Group(NAN_Index,:)=[];
...
for p=1:size(peaks_xyz,1)
    if exist('pa_int_fixed')
        x=WPM_JK12_Memo_TP1_N45.beh_cov_singing(:,2);
        y=cell2mat(pa_int_fixed(:,p+1));
        g=Group(:,5);
        labels=pa_int_fixed(:,1);
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SINGING ALONG MEMO'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM JK12 Memo')
        cd(output_path)
        saveas(gcf,['gscatter_VBM_WPM_Singing_Memo_Cluster_' num2str(p) '.jpg'])
        % label data points
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SINGING ALONG MEMO WITH PATIENTS ID'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM JK12 Memo')
        for k=1:length(labels)
            text (x(k,1), y(k,1), labels {k,1}(5:6),'fontSize',8,'VerticalAlignment','top', 'HorizontalAlignment','left');
        end
        saveas(gcf,['gscatter_VBM_WPM_Singing_Memo_Cluster_' num2str(p) '_labels.jpg'])
    else
%% Plot Intensity against behaviour for the peak coordinate
%%
        x=WPM_JK12_Memo_TP1_N45.beh_cov_singing(:,2);
