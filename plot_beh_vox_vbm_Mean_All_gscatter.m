%% Grey Matter Volume in Baseline Chronic Stroke with Aphasia: data visualization for WAB Mean_All
% *BACKGROUND*
% 
% In this script we will plot the GMV values for the peak voxel in the clusters 
% thresholed at p<0.001 unc and k=50 (Table 1).The multiple regression analyses 
% was set with* words per minute of WAB Mean_All from the onset of the patient's production until the end. *
% 
%% User defined inputs
...
peak_path='G:\Aphasia_project\VBM_v4\analysis\L2\speech\GMV_CPM_Mean_All_TIV_Age_TP1\';
...
% Save peak Voxel in mm and loop for the clusters from different L2 analysis
        cd(peak_path)
        peaks_xyz=xlsread('CPM_Mean_All_TIV_Age_p001unc_k50', 2, 'I2:K6');
        % Loop for patients
...
%%
CPM_Mean_All_TP1_N45= load(fullfile(covbeh_path,'Beh_speech_CPM_Mean_TP1_N45.mat'));
Group=xlsread(fullfile(group_path,'LASA_aphasia_group.xlsx'));

% Remove patients with NaN values in behaviour
if sum(isnan(CPM_Mean_All_TP1_N45.beh_cov_speech(:,2)))~=0
    NAN_Index=find(isnan(CPM_Mean_All_TP1_N45.beh_cov_speech(:,2)));
    CPM_Mean_All_TP1_N45.beh_cov_speech(NAN_Index,:)=[];
    Group(NAN_Index,:)=[];
...
for p=1:size(peaks_xyz,1)
    if exist('pa_int_fixed')
        x=CPM_Mean_All_TP1_N45.beh_cov_speech(:,2);
        y=cell2mat(pa_int_fixed(:,p+1));
        g=Group(:,5);
        labels=pa_int_fixed(:,1);
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SPEECH MEAN ALL'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM WAB Mean_All')
        cd(output_path)
        saveas(gcf,['gscatter_VBM_CPM_Mean_All_Cluster_' num2str(p) '.jpg'])
        % label data points
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SPEECH MEAN ALL WITH PATIENTS ID'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM WAB MEAN ALL')
        for k=1:length(labels)
            text (x(k,1), y(k,1), labels {k,1}(5:6),'fontSize',8,'VerticalAlignment','top', 'HorizontalAlignment','left');
        end
        saveas(gcf,['gscatter_VBM_CPM_Mean_All_Cluster_' num2str(p) '_labels.jpg'])
    else
        %% Plot Intensity against behaviour for the peak coordinate
        %%
        x=CPM_Mean_All_TP1_N45.beh_cov_speech(:,2);
        ...
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SPEECH WAB MEAN ALL'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM WAB MEAN ALL')
        cd(output_path)
        saveas(gcf,['gscatter_VBM_CPM_Mean_All Cluster_' num2str(p) '.jpg'])
        % label data points
        figure
        gscatter(x,y,g)
        title ('VBM: GMV vs SPEECH WAB MEAN ALL WITH PATIENTS ID'), ylabel(['GM Intensity Peak Voxel Cluster ' num2str(p)]), xlabel ('WPM WAB MEAN ALL')
        for k=1:length(labels)
            text (x(k,1), y(k,1), labels {k,1}(5:6),'fontSize',8,'VerticalAlignment','top', 'HorizontalAlignment','left');
        end
        saveas(gcf,['gscatter_VBM_CPM_Mean_All Cluster_' num2str(p) '_labels.jpg'])
    end
end